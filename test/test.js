...
describe('Tests for Investing.com unofficial APIs', () => {
  it('should map an array of arrays to array of objects', () => {
    ...
    assert.equal(mockData[0][0], mappedResponse[0].date)
    assert.equal(mockData[0][1], mappedResponse[0].value)
    assert.equal(mockData[1][0], mappedResponse[1].date)
    assert.equal(mockData[1][1], mappedResponse[1].value)
  })
